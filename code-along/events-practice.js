...
let sentence = `${playerName} rolled a total of ${total}`
    // - Store a reference to the "result" un-ordered list element
    let resultHistory = document.querySelector(`.result`)
    // - Insert the sentence to the "result" un-ordered list as a list item (li)
    resultHistory.insertAdjacentHTML(`beforeend`, `<li>${sentence}</li>`)
}
